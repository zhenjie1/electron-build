 ...
 * @returns
 */
function diffVersionHash(
	oldVersion: HashedFolder,
	newVersion: HashedFolder,
	result = new diffVersionHashResult(),
	path = '.'
) {
	...
	if (newVersion.hash === oldVersion.hash) {
		...
		newVersion.children.forEach((item) => {
			...
			if (oldVersion.childrenObject && oldVersion.childrenObject[item.name]) {
				diffVersionHash(
					oldVersion.childrenObject[item.name],
					item,
					result,
					path + '/' + item.name
				)
 ...
 */
function pushdiffVersionHashResult(
...
function hashElement(
	dir_or_file_path: string,
	options: hashElementOptions = {}
): HashedFolderAndFileType {
	const file_or_dir = fs.statSync(dir_or_file_path)
	const base_name = basename(dir_or_file_path)
...
export async function updater(windows?: BrowserWindow) {
	...
	try {
		...
		if (gt(res.data.version, version)) {
			...
			const diff_result = diffVersionHash(hash, res.data.hash as HashedFolder)
			// 写入更新配置文件
			fs.writeFileSync(
				join(tempDirectory, hotPublishConfig.configName + '.json'),
				JSON.stringify(diff_result, null, 2)
			)
			// 下载
			console.log(diff_result)
			if (windows) windows.webContents.send('hot-update-status', updateInfo)
			await Promise.all(
				diff_result.changed.map((item) => {
					return downAndungzip(
						item.hash,
						join(tempDirectory, item.hash),
						res.data.version
					)
				})
			)
			await Promise.all(
				diff_result.added.map((item) => {
					return downAndungzip(
						item.hash,
						join(tempDirectory, item.hash),
						res.data.version
...
function downAndungzip(source: string, targetPath: string, version: string): Promise<boolean> {
	const sourceUrl = `${hotPublishConfig.url}/${
		hotPublishConfig.gzipDirectory + version
	}/${source}.gz?time=${new Date().getTime()}`
